(function () {
  ...
  var welcomeText = settings.welcomeText || "Kumusta! Paano ka namin matutulungan ngayon?";
  ...
  style.textContent = [
    "#desk-widget-stub{position:fixed;bottom:20px;right:20px;display:flex;align-items:center;gap:10px;",
    ...
    "#desk-widget-stub:focus-visible{outline:2px solid #5CC8C2;outline-offset:3px;}",
    "#desk-widget-stub .desk-dot{width:9px;height:9px;border-radius:50%;background:#5CC8C2;",
    "animation:desk-pulse 2.2s infinite;flex-shrink:0;}",
    "#desk-widget-stub .desk-unread{background:#E8A33D;color:#1A1305;font-family:'IBM Plex Mono',monospace;",
    ...
    "#desk-widget-panel{position:fixed;bottom:20px;right:20px;width:360px;max-width:calc(100vw - 32px);",
    "height:520px;max-height:calc(100vh - 48px);background:#1B2129;border:1px solid rgba(237,234,225,.12);",
    ...
    "margin-top:3px;}",
    "#desk-widget-brand-status .desk-dot{width:7px;height:7px;border-radius:50%;background:#5CC8C2;",
    "animation:desk-pulse 2.2s infinite;}",
    "#desk-widget-ticket{font-family:'IBM Plex Mono',monospace;font-size:11px;color:#E8A33D;",
    ...
    "#desk-widget-close:focus-visible{outline:2px solid #5CC8C2;outline-offset:2px;}",

    "#desk-widget-perf{position:relative;height:14px;margin:0 -1px;flex-shrink:0;",
    "background:radial-gradient(circle at 8px 7px, transparent 4.5px, #1B2129 5px) repeat-x, #12151A;",
    ...
    "flex-direction:column;background:#12151A;}",
    ...
    "color:#5CC8C2;margin-bottom:3px;text-transform:uppercase;}",
    ...
    ".desk-msg.contact{background:#E8A33D;color:#1A1305;border-bottom-right-radius:4px;font-weight:500;}",
    ...
    "#desk-widget-chips{display:flex;flex-wrap:wrap;gap:6px;padding:0 18px 12px;background:#12151A;",
    "flex-shrink:0;}",
    ".desk-chip{font-family:'Inter',sans-serif;font-size:12px;color:#5CC8C2;",
    ...
    ".desk-chip:focus-visible{outline:2px solid #5CC8C2;outline-offset:2px;}",

    "#desk-widget-form{display:flex;align-items:center;gap:8px;padding:12px 14px;background:#1B2129;",
    ...
    "#desk-widget-input:focus{border-color:#5CC8C2;}",
    "#desk-widget-send{background:#E8A33D;border:none;width:36px;height:36px;border-radius:10px;",
    "display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;}",
    "#desk-widget-send:hover{filter:brightness(1.08);}",
    "#desk-widget-send:focus-visible{outline:2px solid #5CC8C2;outline-offset:2px;}",

    "#desk-widget-footnote{text-align:center;font-size:10.5px;color:#6B7280;padding:6px 0 10px;",
    "background:#1B2129;flex-shrink:0;}",

    "@media (max-width:420px){#desk-widget-panel{right:12px;left:12px;width:auto;bottom:12px;",
    "max-height:calc(100vh - 24px);}#desk-widget-stub{right:12px;bottom:12px;}}",
  ].join("");
  ...
  panel.innerHTML =
    ...
    '<div><span id="desk-widget-brand-name">Chat with us</span>' +
    '<div id="desk-widget-brand-status"><span class="desk-dot"></span>Nandito kami</div></div>' +
    ...
    '<button class="desk-chip" data-msg="Gusto kong i-track ang order ko">I-track ang order</button>' +
    '<button class="desk-chip" data-msg="May tanong ako tungkol sa billing">Billing</button>' +
    '<button class="desk-chip" data-msg="May issue akong na-encounter">Mag-report ng issue</button>' +
    "</div>" +
    ...
    "</form>" +
    '<div id="desk-widget-footnote">Karaniwang sumasagot sa loob ng ilang minuto</div>';
  ...
  function init() {
    ...
    if (!state.conversationId) {
      appendMessage({ content: welcomeText, senderType: "bot", senderName: "AI Agent" });
    } else {
